...
class SunsetAndSunriseCollectionViewCell: UICollectionViewCell {
    ...
    private func setConstraint() {
        ...
        timeIcon.rightAnchor.constraint(equalTo: rightAnchor, constant: -30).isActive = true
...
extension SunsetAndSunriseCollectionViewCell {
    ...
    public func configureForecast(with: WeatherData) {
        timeIsNow.text = currentTime(zone: with.city.timezone)
        if with.list.first?.sys.pod == "d" {
            title.text = "ЗАКАТ СОЛНЦА"
            icon.image = UIImage(systemName: "sunset.fill")
            timeIcon.image = UIImage(named: "sun.max.fill")
            sunPosition.text = "Закат: " + unixTimeConverter(time: with.city.sunset, zone: with.city.timezone)
        } else {
            title.text = "ВОСХОД СОЛНЦА"
            icon.image = UIImage(systemName: "sunrise.fill")
            timeIcon.image = UIImage(named: "moon.fill")
            sunPosition.text = "Восход: " + unixTimeConverter(time: with.city.sunrise, zone: with.city.timezone)
        }
    }
}
...
extension SunsetAndSunriseCollectionViewCell {
    ...
    private func unixTimeConverter(time: Int, zone: Int) -> String {
